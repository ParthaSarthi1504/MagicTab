            ...
            <ul className='relative flex justify-between items-center w-[90%] h-[100%] list-none pl-0 '>
                {
                   tabList.map(eachTab=>{
                    if (eachTab.isActive){
                        console.log(eachTab.name,`${95 * (eachTab.id-1)}`)
                    }
                    return(
                        <>
                            ...
                            {
                                eachTab.isActive && (
                                    <div className={`absolute top-[-35px] rounded-full w-[70px] h-[70px] border-[6px] border-black bg-[#00D100] duration-300 translate-x-[${95 * eachTab.id}px] 
